...
class TemplateManager extends AbstractManager
{
    ...
    {
        $query = $this->db->prepare("INSERT INTO templates (category_id, name, description, image_path, created_at, updated_at) VALUES(:category_id, :name, :description, :image_path, :created_at, :updated_at)");
        $parameters = [
    ...
    {
        $query = $this->db->prepare("UPDATE templates SET category_id = :category_id, name = :name, description = :description, image_path = :image_path, created_at = :created_at, updated_at = :updated_at WHERE template_id = :template_id");
        $parameters = [
            "category_id" => $template->getCategoryId(),
            "name" => $template->getName(),
            "description" => $template->getDescription(),
